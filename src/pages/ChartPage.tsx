...
export function ChartPage() {
  ...
  return (
    <div className="flex flex-col gap-6 px-4 py-6 max-w-lg mx-auto">
      ...
      {avgScore !== null && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4 flex gap-4 items-center">
          <span className="text-4xl">{getMood(Math.round(avgScore)).emoji}</span>
          <div>
            <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
              {avgScore}<span className="text-base font-normal text-gray-400 dark:text-gray-500">/10</span>
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Average over {days} days · {chartData.reduce((s, p) => s + p.count, 0)} entries
            </p>
          </div>
        </div>
      )}
